...
7. **Summary state** — the hand stays on its number (e.g. "Hand #2") with an outcome summary in the status line and a colored dot:
   ...
   The Control Bar goes quiet (Undo only), and the felt prompts **"TAP A SEAT TO DEAL"**. Undo remains live to reverse the close (re-open the showdown overlay, or peel a mis-folded fold-out).
...
- **Turn / River are bound-only** (a single card has no "which card" ambiguity) — rank + optional suit, no footnote or relationship. They alone support a **board-suit count**: re-tapping the suit, or the suit-skinned `×N` buttons in the (otherwise empty) shortcut slot, repeats the suit to mark how many are now on the board — `4ss` (flush draw) / `4sss` … shown as repeated pips on the face. Counts: turn `2–4`, river `3–5`. See `ShorthandReference.md §2`.
