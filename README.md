...
4. **Recording** — both halves are active simultaneously. All recording controls live in a **Control Bar** pinned to the bottom (thumb zone), laid out in **two rows**: a slim **utility row** — **Undo** (left) · **Next Street / End Hand** (right) — above a **full-width primary row** of **action buttons**. When the hand closes the bar goes quiet (Undo only). Several input paths coexist:
   - **Action buttons** (the full-width primary row): act on the highlighted seat, then advance the ring to the next player. Context-aware:
     ...
     - An action button that *closes* the street holds on the acting seat and lights the Next Street button — it never advances the street itself (see §5). On that decisive close the **primary action row hides** (the ring also clears and Next Street pulses), so the only live control is Next Street — there's no seat left to act on. Fix a mistake with **Undo**, which stays live.
...
- Seat buttons show action state visually:
  ...
  - Green border + —: check
  - Red border + ✕: fold
  - Gold pulsing ring: currently highlighted seat (action on them). Exactly one seat pulses at a time; on a decisive street close the ring clears, the Next Street button pulses instead, and the primary action row hides (Undo + Next Street only)
...
- **Explicit `x`** is a first-class card state distinct from a blank frame — pressing `x` shows the `x` immediately and reads `Jx` / `Qx`, even alone. **Auto-`x` on leave:** when you dismiss the picker or move to another bank, any ranked card with no chosen suit defaults to explicit `x` (`Q` → `Qx`, partial `AhK` → `AhKx`) so it reads as "rank, unknown suit," never incomplete. *Exception (preserve texture):* a **full, no-suit hole/flop group** (`QJ`, `Q53`) is left bare so its one-tap texture shortcuts (suited/offsuit, rainbow/mono/two-tone) stay available on return; single cards, pairs, turn/river, and footnote/relationship groups are unaffected (footnote already pads `x` in its caption). Blank (rankless) frames stay blank. Logic: `normalizingUnsuited(_:keepsTexture:)`.
...
- The trash icon clears the group; the gold down-chevron at the top (tap / swipe down) dismisses; **Next** (`›`) advances to the next bank (`✓` on the river).
...
- Collapsed it shows a 3–5 line tail; a **gold chevron** in the header expands it **in place** to the full hand and collapses it back — one height-animated panel (not a separate drawer), so expand and collapse are the same motion reversed and the table above never moves.
...
- **Input:** a **docked numeric keypad** (digits, `⌫`, `✓`) that slides up over the bottom section — the same dock/dismiss pattern as the card picker, dismissed by the gold down-chevron (tap / swipe down) or `✓`. Capped at 3 digits, no leading zero. The table above stays put and tappable. **Re-opening a set value shows it as a preview, but the first digit typed clears it and starts fresh** (same as typing a rank into a full card group); backspacing instead keeps the value and edits it in place.
...
- **Where:** a villain's hole group is appended to the **right of RIVER** in the card strip, which becomes **horizontally scrollable** at showdown (the hero hole + board stay spread and anchored on the first screen; villains scroll into view). A one-time **peek-nudge** surfaces the first villain on appear, pointing the user to the off-screen groups. Villain empty slots carry a **red** "enter now" cue (`#C0392B`) — distinct from the hero's gold board/hole cue, so the cards to capture read as someone else's.
...
- Duplicate-card block — a suit button disables when choosing it would recreate a card already known to be in the hand; spans hole/flop/turn/river + villain groups. A card is "known" when its rank is unambiguous: a bound suit (`Qh`), or a **footnote suit on a pair/trips** (`QQhd` → Q♥ and Q♦ are both dead; `QQc` blocks a second ♣). A mixed-rank footnote (`QJdh`) stays ambiguous and blocks nothing. The dead-card set is the pure `deadCardKeys(in:)`
...
| File | Purpose |
|---|---|
| `Aeches/HandEntryView.swift` | Full hand recording engine, two-row Control Bar (Undo · actions · Next Street), per-street card picker, shorthand transcript, all phase logic; also Edit/Resume rehydration (`rehydrate(from:)`, the inverse of `buildHand`) |
...
isComplete:     Bool       // true = played to a real close (showdown/fold-out); false = set aside via Skip (resumable). Drives `result` + resume-vs-edit; stored because outcome==nil is ambiguous
