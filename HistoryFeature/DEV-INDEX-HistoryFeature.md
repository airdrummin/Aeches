...
| History organization (Phase 7) | **Accordion** (`DisclosureGroup` per session) with per-session hand numbering; per-hand **delete** + edit/resume; per-session **delete** + **rename/edit** (reuse the New Session screen). Delete = confirm + **no renumber**. A DEBUG **day-based session** (header = today's date) scaffolds real headers now | Players think in sessions; hand #s are per-session ordinals. Delete/rename are the missing management verbs. The day scaffold lets us build the grouped UI before the real session/auth flow (which brings session creation/selection + resuming arbitrary past sessions). |
...
Consumers
  ...
  │     └─ session-grouped accordion (delete/rename/resume)    ◀── Phase 7
...
| 7 | Session-grouped History (accordion; delete, rename, resume) | ⬜ Not started | 4, 6 | [DEV-PHASE7-HistoryFeature.md](DEV-PHASE7-HistoryFeature.md) |
